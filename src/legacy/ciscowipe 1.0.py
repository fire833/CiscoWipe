...
def find_element(xpath, data):
    findthing = driver.find_element_by_xpath(xpath)
    findthing.send_keys(data)
    time.sleep(0.25)
    
def click_element(xpath2):
    click = driver.find_element_by_xpath(xpath2)
    click.click()

def print_basic_tech(asset, palletinput, gradeinput, processinput, complianceinput):
    find_element(asset_input, asset)
    time.sleep(1)
    map(find_element, (pallet, grade, next_process, compliance_label), (palletinput, gradeinput, processinput, complianceinput))
...
def print_misc_info(license, random, custom):
    new_misc()
    find_element(misc_textbx, license)
    new_misc()
    find_element(misc_textbx_2, random)
    new_misc()
    find_element(misc_textbx_3, custom)

def submit_tech():
    click_element(submit_asset)
"""
